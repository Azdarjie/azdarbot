...
acil = LINETCR.LINE() 
acil.login(token="ErZcolierNqtiVT2ClV3.aIOvmaXVIaAoWkOpoDI+mW.VpBZm87Y8AxVaGIm01zzzzXaxrmw1jwoizSWgU0YiBM=")
acil.loginResult()

pb1 = LINETCR.LINE() 
pb1.login(token="EqrYkSDealXhak7AEec1.FQ+YZEcvIrJ/SRynwFHxuq.Mnamki/gav965e6cHmA15krfK/PidbnDDNC7UfqFpMY=")
pb1.loginResult()

pb2 = LINETCR.LINE() 
pb2.login(token="EqF2QTmjH5q3GwwSiRaa.1UreLE2U683GLhm+qxotYG.LO5CnDVuXwClWt35GCOsm4eR+jW8S35L2vnjpLyGu+0=")
pb2.loginResult()
...
║Gcancelall
║Ginfo
║Azdar in (Masukin bot)
║Azdar out (Keluarin bot)
║Setlastpoint
║Cctv
║Glink
...
admsa = "u022a15300c6f88068ec0aa0f39d8bf33","u7eaff2cde0958918feb2b60d59f85751","uffd77c05bb36006790598d0b3adb6ce1","uaed241eb5533528df8ced098a1f0bd69","ud6e44495edaeebcde094733f417393ca","u5b7dfcf9c66b6a211dd99b25e431a1dd"
...
wait = {
    ...
    'timeline':False,
    'autoAdd':True,
    'message':"""⏩ Azdar Team Bot ⏪""",
    "lang":"JP",
    "comment1":"⏩ Azdar Team Bot ⏪",
    "comment":"Thanks For Add ⏩ Azdar Team Bot ⏪",
    "commentOn":False,
    "commentBlack":{},
    "wblack":False,
    "dblack":False,
...
def bot(op):
    try:
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                msg.to = msg.from_
                if msg.from_ == "u022a15300c6f88068ec0aa0f39d8bf33":
                    if "join:" in msg.text:
        ...
        if op.type == 25:
            ...
            if msg.contentType == 13:
                ...
                msg.contentType = 13
                msg.contentMetadata = {'mid': 'u022a15300c6f88068ec0aa0f39d8bf33'}
                acil.sendMessage(msg)
...
#                else:
                ...
                if len(string.decode('utf-8')) <= 20:
                    ...
                    acil.sendText(msg.to,"􀜁􀇔􏿿semua nama ⏩ Azdar Team Bot ⏪ telah di update menjadi\n👉 " + string + "👈")
...
#--------------------------------------------------------
                ...
                acil.sendText(msg.to,"⏩ Azdar Team Bot ⏪")
                msg.contentType = 13
                msg.contentMetadata = {'mid': 'u022a15300c6f88068ec0aa0f39d8bf33'}
                acil.sendMessage(msg)
                acil.sendText(msg.to,"⏩™✦℘ґ∂ηкℬ◎τ тєαм✦™⏪")
                ...
                for i in gid:
                    ...
                    ki6.leaveGroup(i)
                if wait["lang"] == "JP":
                    acil.sendText(msg.to,"⏩ Azdar Team Bot ⏪ Sudah Keluar Di semua grup")
                else:
...
#---------------------- = NUKE = ------------------
                ...
                if msg.toType == 2:
                    ...
                    start = time.time()
                    pb1.sendText(msg.to, "Cleaning Speed...")
                    elapsed_time = time.time() - start
                    pb1.sendText(msg.to, "%sseconds" % (elapsed_time))
                    pb2.sendText(msg.to, "Cleaning Start...")
                    acil.sendText(msg.to, "Maaf roomnya dibersihkan")
                    acil.sendText(msg.to,"⏩ Azdar Team Bot ⏪")
                    targets = []
                    for g in gs.members:
                        ...
                        for target in targets:
                          if not target in admsa:
                            try:
                                ...
                                acil.sendText(msg,to,"Cleaning Finish")
                                pb2.sendText(msg,to,"Succes Bosku")
#-------------------- = NUKE FINISH = ----------------------------- 
...
#-----------------------------------------------
#-----------------------------------------------
            elif msg.text.lower() == 'azdar out':
                if msg.toType == 2:
                    ginfo = acil.getGroup(msg.to)
                    try:
                        pb1.sendText(msg.to,"⏩™✦℘ґ∂ηкℬ◎τ тєαм✦™⏪\n\nsampai jumpa muaach... 😘\n\n"  +  str(ginfo.name)  + "")
