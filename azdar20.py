                ...
                try:
                    if cctv['cyduk'][op.param1]==True:
                        if op.param1 in cctv['point']:
                            ...
                            Name = kf.getContact(op.param2).displayName
                            Name = kg.getContact(op.param2).displayName
                            Name = kh.getContact(op.param2).displayName
                            Name = kj.getContact(op.param2).displayName
                            Name = kl.getContact(op.param2).displayName
                            Name = km.getContact(op.param2).displayName
                            Name = kn.getContact(op.param2).displayName
                            Name = ko.getContact(op.param2).displayName
                            Name = kp.getContact(op.param2).displayName
                            Name = kq.getContact(op.param2).displayName
                            Name = kr.getContact(op.param2).displayName
                            Name = ks.getContact(op.param2).displayName
                            Name = kt.getContact(op.param2).displayName
		...
		try:
		    if msg.from_ in Creator:
                        for i in gid:
                            ...
                            h = kg.getGroup(i).name
                            h = kh.getGroup(i).name
                            h = kj.getGroup(i).name
                            h = kl.getGroup(i).name
                            h = km.getGroup(i).name
                            h = kn.getGroup(i).name
                            h = ko.getGroup(i).name
                            h = kp.getGroup(i).name
                            h = kq.getGroup(i).name
                            h = kr.getGroup(i).name
                            h = ks.getGroup(i).name
                            h = kt.getGroup(i).name
		            if h == ng:
...
#		else:                    
		...
		cl.sendText(msg.to,"⏩ Azdar Team Bot ⏪")
                ...
                kd.sendText(msg.to,"Bagian Tikung Ada Boss 😌")
                ko.sendText(msg.to,"Iyah Boss Ada 😂")
                kp.sendText(msg.to,"Adaka Juga Boss 🙄")
                kq.sendText(msg.to,"Siap Nikung Boss 😃")
                kr.sendText(msg.to,"Adakah Boss 😍😍")
                ks.sendText(msg.to,"No.16 Siap Boss 🤔")
		...
		cl.sendText(msg.to, "Now Loading...")
            ...
            elif "Percepat" in msg.text:
                cl.sendText(msg.to, "Ok Wait Boss...")
                start = time.time()
                time.sleep(0.000001)
                elapsed_time = time.time() - start
                cl.sendText(msg.to, "%sseconds" % (elapsed_time))  
		...
		if msg.from_ in Creator:
            ...
            elif msg.text.lower() in ["assalamu alaikum","assamu alaikum wr wb"]:
		...
		if msg.from_ in admin:
		    ...
		    if msg.from_ in Creator:
                     if msg.toType == 2:
                        ...
                        gs = kk.getGroup(msg.to)
                        gs = kc.getGroup(msg.to)
                        ki.sendText(msg.to,"Room Cleaning~")
                        kd.sendText(msg.to,"Dadaaah~")
                        targets = []
                        for g in gs.members:
